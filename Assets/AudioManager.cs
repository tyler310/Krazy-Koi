...
public class AudioManager : MonoBehaviour {
	...
	IEnumerator BeatLoader(){
		...
		//Choose Random Angle
		int randAngle = Random.Range(0,360);
		if (audioSource.time > 43.5f) {
			//END
			gameController.GetComponent<GameManager>().EndGame ();
		} else {
			float nextBeatTime = SongArray[noteCount].time;
		    Color nextBeatColor = 
                //Random.value > 0.5 ? Color.white : Color.black;
                SongArray[noteCount].colour == 1? Color.black : Color.white;
		    float nextBeatAngle = 
                //randAngle;
                SongArray[noteCount].angle;
...
//	    {
//	        ColorArray[i] = i%2 == 0? Color.black : Color.white;
//            AngleArray[i] = (10 * i);
//	    }
